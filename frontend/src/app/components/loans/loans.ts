...
export class LoansComponent {
  ...
  constructor() { afterNextRender(() => { const requested = this.route.snapshot.queryParamMap.get('status') as LoanStatus | null; if (requested && ['ALL', 'ACTIVE', 'OVERDUE'].includes(requested)) this.status.set(requested); const requestedMember = Number(this.route.snapshot.queryParamMap.get('memberId')); if (requestedMember) this.memberId.set(requestedMember); this.loadLoans(); }); }
  loadLoans(): void { this.loading.set(true); this.error.set(''); this.loanService.list(this.status(), this.currentPage, this.pageSize, this.memberId() || undefined).pipe(finalize(() => this.loading.set(false))).subscribe({ next: (response) => { this.loans.set(response.content); this.totalLoans.set(response.totalElements); }, error: (error) => this.error.set(apiErrorMessage(error)) }); }
  changeStatus(status: LoanStatus): void { this.status.set(status); this.currentPage = 0; this.loadLoans(); }
  clearMemberFilter(): void { this.memberId.set(null); this.currentPage = 0; void this.router.navigate([], { relativeTo: this.route, queryParamsHandling: 'merge', queryParams: { memberId: null } }); this.loadLoans(); }
